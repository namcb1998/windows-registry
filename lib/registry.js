...
var api = {
    ...
    enumAllSubKeyName: function (key) {
        var listSubkey = [];
        var retval = 0;
        var index = 0;
        while (retval === 0) {
            var keyname = new Buffer(256);
            var classname = "";
            var keylen = ref.alloc(types.LPDWORD, new Buffer(ref.sizeof.pointer));
            var classlen = ref.alloc(types.LPDWORD, new Buffer(ref.sizeof.pointer));
            retval = advApi.RegEnumKeyExA(key.handle.deref(), index, keyname, keylen, null, classname, classlen, null);
            index = index + 1;
            listSubkey.push(keyname.toString().replace(/(\r\n|\n|\r)/gm, "").substring(0, keylen.readUInt32LE()));
        }
        return listSubkey;
    }
